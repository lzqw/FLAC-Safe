...
if [[ -z "$GROUP" || -z "$SEED" ]]; then
  echo "Usage: bash scripts/run_pg2_local_tuning.sh PG2_A1|PG2_A2|PG2_A3|PG2_A4|PG2_A5 seed" >&2
  exit 2
fi
...
case "$GROUP" in
  PG2_A1|PG2_A1_less_conservative)
    GROUP_NAME="PG2_A1_less_conservative"
    SAFE_THRESHOLD="0.10"
    LAMBDA_SAFE="0.7"
    LAMBDA_JVP="0.003"
    ;;
  PG2_A2|PG2_A2_stronger_safe)
    GROUP_NAME="PG2_A2_stronger_safe"
    SAFE_THRESHOLD="0.05"
    LAMBDA_SAFE="1.0"
    LAMBDA_JVP="0.003"
    ;;
  PG2_A3|PG2_A3_stronger_jvp)
    GROUP_NAME="PG2_A3_stronger_jvp"
    SAFE_THRESHOLD="0.05"
    LAMBDA_SAFE="0.7"
    LAMBDA_JVP="0.0035"
    ;;
  PG2_A4|PG2_A4_wider_band)
    GROUP_NAME="PG2_A4_wider_band"
    SAFE_THRESHOLD="0.05"
    LAMBDA_SAFE="0.7"
    LAMBDA_JVP="0.003"
    SAFE_BANDWIDTH="0.075"
    ;;
  PG2_A5|PG2_A5_longer_G4)
    GROUP_NAME="PG2_A5_longer_G4"
    SAFE_THRESHOLD="0.05"
    LAMBDA_SAFE="0.7"
    LAMBDA_JVP="0.003"
    NUM_STEPS="200000"
...
COMMON_ARGS="\
  ...
  --steps 1 \
  --seed ${SEED} \
  --algo MF_SCTD_PG2_Local_Tuning \
  --tag ${TAG}"

{
  echo "===== $(date '+%F %T') START ${GROUP_NAME} seed=${SEED} task=SafetyPointGoal2-v0 tag=${TAG} ====="
  echo "TASK=SafetyPointGoal2-v0 GROUP=${GROUP_NAME}"
...
echo "===== $(date '+%F %T') END ${GROUP_NAME} seed=${SEED} task=SafetyPointGoal2-v0 tag=${TAG} =====" | tee -a "$LOG_FILE"
