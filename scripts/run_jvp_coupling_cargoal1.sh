...
if [[ -z "$GROUP" || -z "$SEED" ]]; then
  echo "Usage: bash scripts/run_jvp_coupling_cargoal1.sh JSC_CG1_1|JSC_CG1_2|JSC_CG1_3|JSC_CG1_4 seed" >&2
  exit 2
fi
...
SAFETY_Q_EXTRA_UPDATES="${SAFETY_Q_EXTRA_UPDATES:-0}"
SAFETY_Q_BOUNDARY_WIDTH="0.05"
SAFETY_Q_FD_EPS="0.01"
DIAGNOSE_SAFETY_Q_GEOMETRY="True"

case "$GROUP" in
  JSC_CG1_1|JSC_CG1_1_C2_safe045_bw025)
    GROUP_NAME="JSC_CG1_1_C2_safe045_bw025"
    LAMBDA_SAFE="0.45"
    SAFE_BANDWIDTH="0.025"
    ;;
  JSC_CG1_2|JSC_CG1_2_C2_safe045_bw030)
    GROUP_NAME="JSC_CG1_2_C2_safe045_bw030"
    LAMBDA_SAFE="0.45"
    SAFE_BANDWIDTH="0.030"
    ;;
  JSC_CG1_3|JSC_CG1_3_C2_safe050_bw025)
    GROUP_NAME="JSC_CG1_3_C2_safe050_bw025"
    LAMBDA_SAFE="0.50"
    SAFE_BANDWIDTH="0.025"
    ;;
  JSC_CG1_4|JSC_CG1_4_G4_safe060_bw025)
    GROUP_NAME="JSC_CG1_4_G4_safe060_bw025"
    LAMBDA_SAFE="0.60"
    SAFE_BANDWIDTH="0.025"
    ;;
  *)
...
COMMON_ARGS="\
  ...
  --lambda_jvp_schedule ${LAMBDA_JVP_SCHEDULE} \
  --lambda_jvp_start ${LAMBDA_JVP_START} \
  --lambda_jvp_end ${LAMBDA_JVP_END} \
  --lambda_jvp_warmup_steps ${LAMBDA_JVP_WARMUP_STEPS} \
  --lambda_jvp_ramp_steps ${LAMBDA_JVP_RAMP_STEPS} \
  --jvp_warmup_steps ${JVP_WARMUP_STEPS} \
  ...
  --steps 1 \
  --seed ${SEED} \
  --algo MF_SCTD_JVP_Coupling_CarGoal1 \
  --tag ${TAG}"

{
  echo "===== $(date '+%F %T') START ${GROUP_NAME} seed=${SEED} task=${TASK} tag=${TAG} ====="
  echo "TASK=${TASK} GROUP=${GROUP_NAME}"
  ...
  echo "lambda_safe=$LAMBDA_SAFE lambda_jvp=$LAMBDA_JVP lambda_jvp_schedule=$LAMBDA_JVP_SCHEDULE lambda_jvp_start=$LAMBDA_JVP_START lambda_jvp_end=$LAMBDA_JVP_END"
  echo "safe_threshold=$SAFE_THRESHOLD safe_bandwidth=$SAFE_BANDWIDTH jvp_warmup_steps=$JVP_WARMUP_STEPS"
  echo "high_fidelity_safety_q=$HIGH_FIDELITY_SAFETY_Q safety_q_priority=$SAFETY_Q_PRIORITY diagnose_safety_q_geometry=$DIAGNOSE_SAFETY_Q_GEOMETRY"
  echo "soft_feasibility_gate=False risk_side_gate=False directional_ref_noise=False"
} | tee "$LOG_FILE"

python main.py $COMMON_ARGS 2>&1 | tee -a "$LOG_FILE"
echo "===== $(date '+%F %T') END ${GROUP_NAME} seed=${SEED} task=${TASK} tag=${TAG} =====" | tee -a "$LOG_FILE"
