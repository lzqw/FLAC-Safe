...
session_name() {
  local group="${1%%:*}"
  local seed="${1##*:}"
  echo "cargoal1_tune_${group}_${seed}"
}
...
bash scripts/run_cargoal1_local_tuning.sh ${group} ${seed}"
  echo "started $sess"
  return 0
}
...
run_all() {
  ...
  if (( ${#queue[@]} == 0 )); then
    echo "no pending CarGoal1 local tuning runs"
    monitor_once
    python scripts/collect_cargoal1_local_tuning.py || true
    run_status_check
    return 0
  fi
  n="$(parallel_n)"
  ...
  while (( idx < ${#queue[@]} )); do
    ...
    for spec in "${queue[@]}"; do
      if failed_log "$spec"; then
        echo "error detected in $spec; not launching additional CarGoal1 local tuning runs"
        python scripts/collect_cargoal1_local_tuning.py || true
        return 1
      fi
    done
...
stop_sessions() {
  for sess in $(tmux ls 2>/dev/null | awk -F: '/^cargoal1_tune_/ {print $1}'); do
...
case "$MODE" in
  ...
  stop)
    ...
    ;;
  *)
    echo "Usage: bash scripts/launch_cargoal1_local_tuning_dynamic.sh plan|all|status|stop" >&2
    exit 2
    ;;
esac
