...
group_name() {
  case "$1" in
    SG_CG1_1|SG_CG1_1_C2_tau010_floor02) echo "SG_CG1_1_C2_tau010_floor02" ;;
    SG_CG1_2|SG_CG1_2_C2_tau010_floor03) echo "SG_CG1_2_C2_tau010_floor03" ;;
    SG_CG1_3|SG_CG1_3_G4_tau010_floor02) echo "SG_CG1_3_G4_tau010_floor02" ;;
    SG_CG1_4|SG_CG1_4_C2_tau005_floor02) echo "SG_CG1_4_C2_tau005_floor02" ;;
    *) echo "$1" ;;
  esac
}
...
queue_specs() {
  printf '%s\n' \
    SG_CG1_1:0 SG_CG1_1:1 \
    SG_CG1_2:0 SG_CG1_2:1 \
    SG_CG1_3:0 SG_CG1_3:1 \
    SG_CG1_4:0 SG_CG1_4:1
}
...
session_name() {
  local group="${1%%:*}"
  local seed="${1##*:}"
  echo "cargoal1_softgate_${group}_${seed}"
}
...
bash scripts/run_cargoal1_soft_gate.sh ${group} ${seed}"
  echo "started $sess"
  return 0
}
...
run_status_check() {
  ...
  echo "===== progress ====="
  grep -E "Env: SafetyCarGoal1-v0|Episode:|total numsteps| START | END " "$LOG_DIR"/*.log 2>/dev/null | tail -n 160 || true
  echo "===== failed logs ====="
...
run_all() {
  ...
  if (( ${#queue[@]} == 0 )); then
    echo "no pending CarGoal1 soft-gate runs"
    monitor_once
    python scripts/collect_cargoal1_soft_gate.py || true
    run_status_check
    return 0
  fi
  n="$(parallel_n)"
  ...
  while (( idx < ${#queue[@]} )); do
    ...
    for spec in "${queue[@]}"; do
      if failed_log "$spec"; then
        echo "error detected in $spec; not launching additional CarGoal1 soft-gate runs"
        python scripts/collect_cargoal1_soft_gate.py || true
        return 1
      fi
    done
...
stop_sessions() {
  for sess in $(tmux ls 2>/dev/null | awk -F: '/^cargoal1_softgate_/ {print $1}'); do
...
case "$MODE" in
  ...
  stop)
    ...
    ;;
  *)
    echo "Usage: bash scripts/launch_cargoal1_soft_gate_dynamic.sh plan|all|status|stop" >&2
    exit 2
    ;;
esac
