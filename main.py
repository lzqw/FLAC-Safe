...
if __name__ == "__main__":
    ...
    arg.add_arg("safety_critic_mode", "cumulative", "Safety critic target mode: cumulative or cdf")
    arg.add_arg("qc_geom_mode", "max", "Safety critic geometry scalar mode for JVP: max or mean")
    arg.add_arg("cdf_binarize_cost", True, "Binarize cost before CDF safety target")
    ...
    arg.add_arg("feas_gate_reward_floor", 0.2, "Minimum reward weight under soft feasibility gate")
    arg.add_arg("soft_normal_masking", False, "Enable soft normal masking for safety geometry")
    arg.add_arg("masking_warmup_steps", 20000, "Environment steps before enabling soft normal masking")
    arg.add_arg("mask_beta_max", 0.5, "Maximum soft masking beta")
    ...
    arg.add_arg("safety_q_priority", False, "Enable cost/boundary-prioritized safety-Q critic loss")
    arg.add_arg("safety_q_cost_weight", 2.0, "Safety-Q priority weight for cost samples")
    arg.add_arg("safety_q_boundary_weight", 3.0, "Safety-Q priority weight for near-boundary samples")
    arg.add_arg("safety_q_td_weight", 0.0, "Safety-Q priority weight for TD-error magnitude")
    arg.add_arg("safety_q_max_weight", 5.0, "Maximum safety-Q priority sample weight")
    arg.add_arg("safety_q_extra_updates", 0, "Extra safety-critic-only updates per normal update")
    arg.parser()
